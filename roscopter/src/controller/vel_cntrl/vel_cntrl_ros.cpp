  ...
  nh_(ros::NodeHandle()), nh_private_("~")
{
  ...
  state_sub_ = nh_.subscribe("estimate", 1, &Vel_Cntrl_Ros::stateCallback, this);
...
void Vel_Cntrl_Ros::init_controller()
{
  if (debug_init_controller_)
    std::cout << "In Vel_Cntrl_Ros::init_controller!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
...
void Vel_Cntrl_Ros::stateCallback(const nav_msgs::OdometryConstPtr &msg)
{
  if (debug_stateCallback_)
    std::cout << "In Vel_Cntrl_Ros::stateCallback!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
  ...
  if(is_flying_ && armed_ && received_cmd_)
  {
    ...
    control.computeVelocityControl(dt);
    publishCommand();
  }
  else
  {
...
void Vel_Cntrl_Ros::isFlyingCallback(const std_msgs::BoolConstPtr &msg)
{
  if (debug_isFlyingCallback_)
    std::cout << "In Vel_Cntrl_Ros::isFlyingCallback!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
  is_flying_ = msg->data;
}

void Vel_Cntrl_Ros::statusCallback(const rosflight_msgs::StatusConstPtr &msg)
{
  if (debug_statusCallback_)
    std::cout << "In Vel_Cntrl_Ros::statusCallback!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
  armed_ = msg->armed;
}

void Vel_Cntrl_Ros::cmdCallback(const rosflight_msgs::CommandConstPtr &msg)
{
  if (debug_cmdCallback_)
    std::cout << "In Vel_Cntrl_Ros::cmdCallback!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
...
void Vel_Cntrl_Ros::reconfigure_callback(roscopter::ControllerConfig& config,
                                      uint32_t level)
{
  if (debug_reconfigure_callback_)
    std::cout << "In Vel_Cntrl_Ros::reconfigure_callback!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
...
void Vel_Cntrl_Ros::publishCommand()
{
  if (debug_publishCommand_)
    std::cout << "In Vel_Cntrl_Ros::publishCommand!!!!!!!!!!!!!!!!!!!!!!!!!!! \n";
